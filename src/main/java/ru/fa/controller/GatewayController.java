...
public class GatewayController {
    ...
    public ResponseEntity<?> routeGetCommoditiesRequest(ProxyExchange<Object> proxy,
                                                    @RequestHeader(HttpHeaders.AUTHORIZATION) String auth,
                                                    @RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie,
                                                    @RequestBody(required = false) Object body) {
        try {
            String path = proxy.path("/commodities/");
            return proxy.uri("http://localhost:8083/" + path)
                    .header(HttpHeaders.AUTHORIZATION, auth)
                    .header(HttpHeaders.COOKIE, cookie)
                    .header(HttpHeaders.CONTENT_TYPE, "application/json")
                    .body(body)
    ...
    public ResponseEntity<?> routePostCommoditiesRequest(ProxyExchange<Object> proxy,
                                                     @RequestHeader(HttpHeaders.AUTHORIZATION) String auth,
                                                     @RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie,
                                                     @RequestBody(required = false) Object body) {
        try {
    ...
    public ResponseEntity<?> routeDeleteCommoditiesRequest(ProxyExchange<Object> proxy,
                                                       @RequestHeader(HttpHeaders.AUTHORIZATION) String auth,
                                                       @RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie,
                                                       @RequestBody(required = false) Object body) {
        try {
